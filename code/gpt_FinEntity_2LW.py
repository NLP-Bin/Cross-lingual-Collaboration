...
def gpt(start, num, model_choice):
...
"""
    ...
    # 不好
    com_a1_a2_prompt_v3 = """Please refer to the Chinese result to revise the English result, and provide the final reasonable English result. 
...
Only output the final English result. """
    ...
    save_path = '../data/2LW_output/2LW_3.5-open_ai_FinEntity_0shot_test_promptV3+V3+V3+NUM'
    ...
    try:
        ...
        for index_all, item in enumerate(tqdm(raw, desc="Processing items")):
            ...
            # prompt2 = Translation_EFSA_prompt_V4
            ...
            #并行执行任务
            prompt2 = Translation_EFSA_prompt_V4.format(sentence=sentence)
            messages_Ts_task = [
                {"role": "system", "content": sys_prompt},
                {"role": "user", "content": prompt2},
                # {"role": "user", "content": sentence},
            ]
            # messages_Ts_task.append({"role": "user", "content": prompt2})
            # print("\n\nmessages:\n\n", messages)
            try:
                rsp2 = openai.ChatCompletion.create(
                    model=model_choice,
                    messages=messages_Ts_task,
                    temperature=0.0,
                )
            ...
            # 如果第二步并行2：
            prompt3 = com_a1_a2_prompt_v4.format(sentence=sentence, EnR=answer1, ZhR=answer2)
            messages_com = [{"role": "user", "content": prompt3},]
            print("\n\nmessages:\n\n", messages_com)
            try:
                rsp3 = openai.ChatCompletion.create(
                    model=model_choice,
                    messages=messages_com,
                    temperature=0.0,
                )
...
# 拼接日志文件名
log_file_name = f"log_{model_choice}_{current_time}.log"
log_file_path = f"../data/2LW_output/{log_file_name}"   # 日志文件路径


compare_list, result_list = gpt(start, num, model_choice)
