...
TEXT:"""
    ...
    Translation_EFSA_prompt = """\nTranslate the financial text accurately into Spanish, reconsider it from the Spanish perspective, and perform a more comprehensive entity-level sentiment analysis. 
...
Translation: 
Output: """
    Translation_EFSA_prompt_V2 = """\nTranslate financial texts into accurate Chinese and review the sentiment polarity of the extracted financial entities from the perspective of Chinese. 
    Check whether all sentiment polarities are correct and whether there are any missing financial entities. 
    Output the corrected quadruples (do not annotate financial terms without specific reference).
    Provide the translation and the new quadruple output in the following format:
    Translation: 
    Output: """
    #不好
    ...
    Output: """
    #这个结合V3+V3+V3可以
    Translation_EFSA_prompt_V35 = """\nTranslate the financial text accurately into Chinese, reconsider it from the Chinese perspective, and perform a more comprehensive entity-level sentiment analysis. 
    Consider entities such as organizations, companies, products, brands, etc. Filter out meaningless generic terms.
    Provide the translation and the new quadruple output in the following format:
    Translation: 
    Output: """

#     Translation_EFSA_prompt_V4 = """\nTranslate the financial text accurately into Chinese, reconsider it from the Chinese perspective, and perform a more comprehensive entity-level sentiment analysis.
#
#     Identify entities in the content that belong to companies or organizations and classify their corresponding sentiment into ‘Neutral,’ ‘Positive,’ or ‘Negative.’. Stock codes of companies do not need to be marked.
# Consider each sentence as a string in Python, and provide the start and end indices (zero-based indexing) to mark the boundaries of the entities, including spaces and punctuation.
# Do not provide any explanation for the sentiment classification. In the output, “Tag” represents the sentiment, and “value” represents the entity name. If no entity is found in a sentence, the output should be empty.
# Output format example: {"start": 0, "end": 7, "value": "中文金融实体", "tag": "Neutral"}.
# Use line breaks to separate different quadruples.
#
#     The sentence may contain varying numbers of financial entities. Do not mark general or irrelevant content such as "customer" or "company," as well as countries, personal names, dates, and job titles.
# Please pay attention to the output format.
# Provide the translation and the quadruple output in the following format:
#     Translation:
#     Output:
# TEXT: """
...
"""
    ...
    # com_a1_a2_prompt = """After determining the sentiment polarity of the samples above, which are the same financial entity in Chinese and English, only English is retained, and different financial entities are retained.
    # The final result is given: """
    #还行！
    # com_a1_a2_prompt_v2 = """Integrate the above results, keeping only the English samples of the same financial entity and considering whether the sentiment polarity is reasonable. If it is unreasonable, please correct it.
    # If the Chinese results mention new entities, convert them into the English results and keep them.
    # Delete redundant results. Provide the final results."""
    # 不好
    ...
    Ensure that an appropriate sentiment polarity is provided for each occurrence of an entity in the English text. Stock codes(e.g., JNJ.N,) of companies and  Wall Street do not need to be marked. If they are found, please delete them.
    Only output the result, do not output any irrelevant content."""
    #

    com_a1_a2_prompt_v4 = """Perform a task of screening and merging the Chinese and English quadruple results for financial entity-level sentiment analysis:
"Financial entities" refers to companies or organizations and does not include stock codes, countries, personal names, etc. If found, please delete them.
For the English financial text: {sentence}
Its English result is: {EnR}
Its Chinese result is: {ZhR}
Only output the final English result. """

    save_path = f'../data/2LW_output/{to_language}/2LW_pipeline_{model_choice}_FinEntity_{to_language}_0shot_test+NUM'
    # save_path = '../data/2LW_output/2LW_pipeline_4o-open_ai_FinEntity_French_0shot_test_promptV3+V3+V3+NUM'
...
def get_compare_from_file(to_language, model_choice, num):
    raw = json.load(open('../data/input/FinEntity.json', 'r', encoding='utf-8'))
    raw = raw[:num]
    compare_list = []
    for item in raw:
        compare_list.append(item)
    result_list = json.load(open(F'../data/2LW_output/{to_language}/2LW_pipeline_{model_choice}_FinEntity_{to_language}_0shot_test+NUM'+str(num)+'.json', 'r', encoding='utf-8'))[:num]
    print(len(compare_list))
    print(len(result_list))
...
for to_language in to_language_list:
    ...
    log_file_path = f"../data/2LW_output/{to_language}/{log_file_name}"   # 日志文件路径
