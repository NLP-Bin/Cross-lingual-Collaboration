...
#        The sentence: "Investors should avoid power sector till the Lok Sabha elections: G Chokkalingam, Equinomics. "
#        """
    # The output format strictly follows the following
    # form: {\"value\": \"power sector\", \"start\": 23, \"end\": 35, \"tag\": \"negative\"}\n{\"value\": \"Equinomics\", \"start\": 82, \"end\": 92, \"tag\": \"neutral\"}
    assist_prompt = """{\"value\": \"power sector\", \"start\": 23, \"end\": 35, \"tag\": \"negative\"}\n{\"value\": \"Equinomics\", \"start\": 82, \"end\": 92, \"tag\": \"neutral\"}
        """
    user_prompt2 = """Dependence on power sector may hit BHELs margins
        """
    assist_prompt2 = """{\"value\": \"BHEL\", \"start\": 35, \"end\": 39, \"tag\": \"negative\"}
        """

    user_prompt3 = """Unichem Lab Q1 net up 9 per cent at Rs 36 crore
        """

    assist_prompt3 = """{\"value\": \"Unichem Lab\", \"start\": 0, \"end\": 11, \"tag\": \"positive\"}
        """
    ...
    for item in raw:
        ...
        try:
            ...
            result_list.append(result_dict)
            compare_list.append(item)
    with open('../data/output/4o-open_ai_SEntFiN_0_911.json', 'wt') as f:
    ...
    with open('../data/output/4o-open_ai_SEntFiN_result_0_911.json', 'wt') as f:
        print(json.dumps(result_list,indent=2), file=f)
    print("COMPLETE")
...
for example in result_list:
    for annotation in example['annotations']:
        #We expect the key of label to be label but the data has tag
        annotation['label'] = annotation['tag']
    with open('../data/output/4o-open_ai_SEntFiN_result.json', 'wt') as f:
        print(json.dumps(result_list,indent=2), file=f)
